	...
	for (i = 0; i < kNumBalls; i++)
        ...
        { // YOUR CODE HERE
            ...
            if(ballsDistBetween <= 0.0){
                ...
                float relativeScalar = DotProduct(VectorSub(ball[i].v, ball[j].v), ballsCenterToCenterDiff); //v_rel (dot) p , (GJK?)

                if(relativeScalar <= 0.0) { //balls are not seperating
                    ...
                    vec3 imp2 = ScalarMult(imp, -1.0);
                    //send balls in oposite directions
                    ball[i].P = VectorAdd(ball[i].P, imp);
                    ball[j].P = VectorAdd(ball[j].P, imp2);
