  ...
  int row, corner;
    ...
    for (int i = 1; i < kMaxBones; i++) {
				if(i > 0){
					...
					// T_rest[i] = T(g_bones[i].pos.x , g_bones[i].pos.y, g_bones[i].pos.z);
				}
    ...
    // // Animated bones(Mbm[row], Mmb[row]
    // M[0] = Mult(primes[0], inverse[0]);
    // for (int i = 1; i < kMaxBones; i++) {
    //     M[i] = Mult(Mult(primes[i], M[i-1]), inverse[i]);
    // }

		Mmb[0] = InvertMat4(T_rest[0]);
		Mbm[0] = Mult(T_rest[0], g_bonesRes[0].rot);

		for (int i = 1; i < kMaxBones; i++) {
			Mmb[i] = Mult(InvertMat4(T_rest[i]), Mmb[i-1]);
			Mbm[i] = Mult(Mult(Mbm[i-1], T_rest[i]), g_bonesRes[i].rot);
		}
    ...
    for (corner = 0; corner < kMaxCorners; corner++)
    {
      ...
      // g_vertsRes
			...
			for (int i = 0; i < kMaxBones; i++) {
				vec3 weightedPos = ScalarMult(MultVec3(MultMat4(Mbm[i], Mmb[i]), g_vertsOrg[row][corner]), g_boneWeights[row][corner][i]);
				sum = VectorAdd(sum ,weightedPos);
			}
